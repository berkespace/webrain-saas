...
async function main() {
  console.log('🌱 Neon veritabanı seed işlemi başlıyor...')
  ...
  try {
    ...
    console.log(`🏪 Komisyoncular: ${await prisma.komisyoncu.count()}`)
    console.log(`🏢 Özel Firmalar: ${await prisma.ozelFirma.count()}`)
    console.log(`🥬 Ürünler: ${await prisma.urun.count()}`)
    console.log(`👨‍🌾 Müstahsiller: ${await prisma.mustahsil.count()}`)
    console.log(`👨‍🌾 Üreticiler: ${await prisma.uretici.count()}`)
    console.log(`📦 Ambalajlar: ${await prisma.ambalaj.count()}`)
    console.log(`📝 Mal Kabul Kayıtları: ${await prisma.malKabulRecord.count()}`)
